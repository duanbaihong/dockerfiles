...
class CephFSNativeDriver(object):
    ...
    def _authorize_ceph(self, volume_path, auth_id, readonly):
        ...
        # read the layout
        pool_name = self._volume_client._get_ancestor_xattr(
            path, "ceph.dir.layout.pool")
        try:
            namespace = self._volume_client.fs.getxattr(
                path, "ceph.dir.layout.pool_namespace")
        ...
        want_mds_cap = 'allow r,allow {0} path={1}'.format(
            want_access_level, path)
        if namespace:
        ...
        try:
            ...
            unwanted_mds_cap = 'allow {0} path={1}'.format(
                unwanted_access_level, path)
            if namespace:
            ...
            osd_cap_str = cap_update(cap['caps'].get(
                'osd', ""), want_osd_cap, unwanted_osd_cap)
            mds_cap_str = cap_update(cap['caps'].get(
                'mds', ""), want_mds_cap, unwanted_mds_cap)
    ...
    def create_share(self, path, user_id, size=None):
        ...
        volume = self.volume_client.create_volume(
            volume_path, size=size, namespace_isolated=not self.ceph_namespace_isolation_disabled)
    ...
    def _deauthorize(self, volume_path, auth_id):
        ...
        pool_name = self.volume_client._get_ancestor_xattr(
            path, "ceph.dir.layout.pool")
        try:
            namespace = self.volume_client.fs.getxattr(
                path, "ceph.dir.layout.pool_namespace")
            if type(namespace) is bytes:
                namespace = namespace.decode('UTF-8')
        except cephfs.NoData:
        ...
        try:
            ...
            if (not osd_cap_str) and (not osd_cap_str or mds_cap_str == "allow r"):
                ...
                self.volume_client._rados_command(
                    'auth del', {'entity': client_entity}, decode=False)
            else:
                self.volume_client._rados_command(
                    'auth caps',
                    {
...
def usage():
    print("Usage: " + sys.argv[0] +
          " --remove -n share_name -u ceph_user_id -s size")
